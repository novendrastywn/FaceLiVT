...
class Classfier(nn.Module):
    ...
    @torch.no_grad()
    def deploy(self):
        classifier = self.classifier.deploy()
        if self.distillation:
            classifier_dist = self.classifier_dist.deploy()
...
class Affine(nn.Module):
    def __init__(self, dim, res=None):
        super().__init__()
        if res is not None :
            self.alpha = nn.Parameter(1e-5 * torch.ones((1, dim, res)), requires_grad=True)
            self.beta = nn.Parameter(1e-5 * torch.zeros((1, dim, res)), requires_grad=True)
        else:
            self.alpha = nn.Parameter(1e-5 * torch.ones((1, dim, 1)), requires_grad=True)
            self.beta = nn.Parameter(1e-5 * torch.zeros((1, dim, 1)), requires_grad=True)
...
class MultiHeadLinearAttention(torch.nn.Module):
    ...
    def __init__(self, dim, resolution):
        ...
        self.norm = Affine(dim) # nn.GroupNorm(1, dim)
        for i in range(self.n_head):
            linear.append(nn.Linear(self.res, self.res))
        self.lin = torch.nn.ModuleList(linear)
        self.ls = nn.Parameter(1e-5 * torch.ones(dim).unsqueeze(-1).unsqueeze(-1), requires_grad=True)


    def forward(self, x):
        B,C,H,W = x.shape
        x = self.norm(x.reshape(-1, self.dim, self.res))
        x = list(x.chunk(self.n_head, dim=1))
        for i in range(self.n_head):
            x[i] = self.lin[i](x[i])
        x = torch.cat(x, dim=1)
        x =  self.ls * x.reshape(B,C,H,W)
        return x #self.drop_path(x) 
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio,  resolution, type, act_layer=nn.ReLU):
        ...
        if type=='repmix':
            ...
            rep_mixer   = RepConv(dim, dim, ks=3, stride=1, pad=1, groups=dim)
            mhla_mixer  = MultiHeadLinearAttention(dim, resolution)
            # mhla_mixer  = MultiHeadMLPAttention(dim, resolution)
            self.block  = nn.Sequential(
                            Residual(rep_mixer),
                            Residual(mhla_mixer),
                            Residual(FFN(dim, dim*mlp_ratio, act_layer)))
...
class FaceLiVTv2(nn.Module):  
    def __init__(self, in_chans=3, img_size=112,
        ...
        final_feature_dim = final_feature_dim if final_feature_dim is not None else dims[-1]
        if pre_head == "gdconv":
            self.pre_head = nn.Sequential(
                          Conv2d_BN(dims[-1],final_feature_dim) \
                          if final_feature_dim is not None else nn.Identity(),
                          Conv2d_BN(final_feature_dim,final_feature_dim, 4, groups=final_feature_dim)
                          ) 
        elif pre_head == "gap":
            self.pre_head = nn.AdaptiveAvgPool2d(1)
        else:
            print("type of head is not listed")
        
        self.head = Classfier(final_feature_dim, num_classes, distillation)
...
@register_model
def facelivtv2_xs(num_classes = 512, distillation=False, pretrained=True, **kwargs):
    model=FaceLiVTv2(
...
@register_model
def facelivtv2_s(num_classes = 512, distillation=False, pretrained=True, **kwargs):
    model=FaceLiVTv2(
            num_classes=num_classes,
            dims=[ 48, 96, 192, 320],
            depths=[ 3, 3, 9, 3],
...
@register_model
def facelivtv2_m(num_classes = 512, distillation=False, pretrained=True, **kwargs):
    model=FaceLiVTv2(
            num_classes=num_classes,
            dims=[ 56, 112, 224, 448],
            depths=[ 3, 3, 9, 3],
...
@register_model
def facelivtv2_l(num_classes = 512, distillation=False, pretrained=True, **kwargs):
    model=FaceLiVTv2(
            num_classes=num_classes,
            dims=[ 64, 128, 256, 512],
            depths=[ 3, 3, 9, 3],
