...
def main(args):
    ...
    # print(backbone)
    if local_rank == 0:
        print(f"Start Training Model: {cfg.network}, GFLOPs: {gmacs:.3f}, Mparams: {(params/(1000**2)):.3f}")
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                callback_logging(global_step, loss_am, loss_am, loss_am, epoch, cfg.fp16, lr_scheduler.get_last_lr()[0], amp)

                if global_step % cfg.verbose == 0 and epoch >= int(cfg.num_epoch*0.75):
